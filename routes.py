...
def index():
    ...
    #     while counter < len(manage_List):
    #         if(counter < (len(manage_List) - 1)):
    #             management_view += (manage_List[counter] + ", ");
    #         else:
    #             management_view += manage_List[counter];
    #         counter += 1;
...
def Recruitment():
    """
    Return the list of employees that are free
    """

    employees = database.get_noDepartment_employee()
...
def add_employee():
    ...
    # Attempt the "revoke"
    success = database.get_noDepartment_employee()
    ...
    page['bar'] = success[0]
    flash("added" if success[0] else success[1])
